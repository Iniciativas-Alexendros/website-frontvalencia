...
async function main() {
  ...
  page.on('response', (r) => {
    ...
    if (u.match(/\.(jpg|jpeg|png|gif|webp|avif|svg)$/i) && !u.includes('metricool') && !u.startsWith('data:'))
      netImages.add(u)
  })
  ...
  for (const { r, n, l } of ROUTES) {
    ...
    try {
      ...
      for (let i = 0; i < Math.min(lcnt, 100); i++) {
        const href = await linkEls
          .nth(i)
          .getAttribute('href')
          .catch(() => '')
        const text = await linkEls
          .nth(i)
          .innerText()
          .catch(() => '')
        const rel = await linkEls
          .nth(i)
          .getAttribute('rel')
          .catch(() => '')
      ...
      for (let i = 0; i < Math.min(icnt, 50); i++) {
        const src = await imgEls
          .nth(i)
          .getAttribute('src')
          .catch(() => '')
        const alt = await imgEls
          .nth(i)
          .getAttribute('alt')
          .catch(() => '')
      ...
      for (let i = 0; i < vcnt; i++) {
        const poster = await vidEls
          .nth(i)
          .getAttribute('poster')
          .catch(() => '')
        const src = await vidEls
          .nth(i)
          .getAttribute('src')
          .catch(() => '')
      ...
      save(key, data)
      console.log(
        `  ✓ ${body.length} chars | ${Object.values(headings).flat().length} headings | ${images.length} imgs | ${videos.length} vids | ${links.length} links`,
      )
    } catch (err: any) {
  ...
  for (const f of [
    ...
    '/arrow-b.png',
    '/arrow.png',
    '/ILUSTRACION_MV_PEQUEÑO.svg',
    '/ILUSTRACION_MV_GRANDE.svg',
    '/ILUSTRACION_TABLET.svg',
    '/ILUSTRACION_WEB-2.svg',
    '/ILUSTRACION_WEB_1250.svg',
    '/ILUSTRACION_WEB_2020.svg',
  ]) {
